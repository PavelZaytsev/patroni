...
class Ha(object):
    ...
    def disable_synchronous_replication(self) -> None:
        ...
        if not self.cluster.sync.is_empty and self.dcs.delete_sync_state(version=self.cluster.sync.version):
            ...
            logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(empty) from disable_synchronous_replication() - sync turned off")
            self.state_handler.sync_handler.set_synchronous_standby_names(CaseInsensitiveSet())
        ...
        logger.info("[SYNC-DEBUG] HA: Calling config.set_synchronous_standby_names('%s') from disable_synchronous_replication() - user config fallback", ssn)
            ...
            for transition, leader, num, nodes in QuorumStateResolver(leader=leader,
                ...
                if transition == 'quorum':
                    ...
                    if num < min_sync:
                        ...
                        num = min_sync
                    logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(%s, %s) from _process_quorum_replication() - quorum sync", list(nodes), num)
    ...
    def _process_multisync_replication(self) -> None:
        ...
        logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(%s) from _process_multisync_replication() - multisync", list(picked))
    ...
    def process_sync_replication_prepromote(self) -> bool:
        ...
        logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(%s, %s) from process_sync_replication_prepromote() - pre-promote", list(sync), numsync)
    ...
    def demote(self, mode: str) -> Optional[bool]:
        ...
        if self.is_synchronous_mode():
            logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(empty) from demote() - demoting")
            self.state_handler.sync_handler.set_synchronous_standby_names(CaseInsensitiveSet())
    ...
    def post_bootstrap(self) -> str:
        ...
        if self.is_synchronous_mode():
            ...
            logger.info("[SYNC-DEBUG] HA: Calling sync_handler.set_synchronous_standby_names(%s) from post_bootstrap() - bootstrap complete, sync_mode_strict=%s", list(sync_value), global_config.is_synchronous_mode_strict)
